...
import { ViewDetailPenerimaan } from "../../types/data"; // Perlu ditambahkan di types
import { fetchDetailPenerimaanData } from "../../services/DataMasterServices"; // Perlu ditambahkan di services

export default function DetailPenerimaanPage() {
  const navigate = useNavigate();
  const [detailList, setDetailList] = useState<ViewDetailPenerimaan[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      setError(null);
      try {
        const data = await fetchDetailPenerimaanData();
        setDetailList(data);
      } catch (err: any) {
  ...
  return (
    <>
      <PageMeta title={`Detail Penerimaan`} description="Detail item pada transaksi pengadaan." />
      <PageBreadcrumb pageTitle={`Detail Penerimaan`} />
      
      <div className="space-y-6">
        <ComponentCard title="Daftar Barang Penerimaan">
                ...
                <Table className="w-full">
                  <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                    <TableRow>
                      ...
                      <TableCell isHeader className="px-5 py-3">Jumlah Terima</TableCell>
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    {detailList.map((item) => (
                      <TableRow key={item.iddetail_penerimaan} className="hover:bg-gray-50 dark:hover:bg-white/5">
                        <TableCell className="px-5 py-4">{item.iddetail_penerimaan}</TableCell>
                        <TableCell className="px-5 py-4 font-medium text-gray-800 dark:text-white/90">{item.nama_barang}</TableCell>
                        <TableCell className="px-5 py-4">{item.jumlah_terima}</TableCell>
                        <TableCell className="px-5 py-4">{item.harga_satuan_terima}</TableCell>
                        <TableCell className="px-5 py-4">{item.sub_total_terima}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
