...
export default function TambahPenjualanPage() {
  ...
  const [persenMargin, setPersenMargin] = useState<number>(
    initialDraft.persenMargin,
  );
  ...
  const [allBarangs, setAllBarangs] = useState<BarangData[]>([]);
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        setAllBarangs(barangs);

        setBarangOptions(
          barangs.map((b) => ({
            value: String(b.idbarang),
            label: `${b.nama} - Harga Beli: Rp ${b.harga?.toLocaleString() || 0}, Stok: ${b.stock}`,
          })),
        );
      } catch (err: any) {
    ...
    const fetchMarginPersen = async () => {
      ...
      try {
        ...
        const selectedMargin = margins.find(
          (m) => m.idmargin_penjualan === parseInt(idMargin),
        );
        if (selectedMargin) {
  ...
  useEffect(() => {
    if (!selectedBarang || allBarangs.length === 0) {
      setSelectedStock(0);
      return;
    }

    const selected = allBarangs.find(
      (b) => b.idbarang === parseInt(selectedBarang),
    );
    if (selected) {
      setSelectedStock(selected.stock);
    }
  }, [selectedBarang, allBarangs]);
  ...
  const handleAddDetail = () => {
    ...
    if (jumlah > selectedStock) {
      alert(
        `Jumlah tidak boleh melebihi stok yang tersedia (${selectedStock})`,
      );
      return;
    }

    const existingItemIndex = details.findIndex(
      (d) => d.idbarang === parseInt(selectedBarang),
    );
    if (existingItemIndex >= 0) {
      const totalJumlah = details[existingItemIndex].jumlah + jumlah;
      if (totalJumlah > selectedStock) {
        alert(
          `Total jumlah tidak boleh melebihi stok yang tersedia (${selectedStock}). Saat ini sudah ada ${details[existingItemIndex].jumlah} item.`,
        );
        return;
      }
      if (
        confirm(
          "Barang ini sudah ada di daftar. Apakah ingin menambahkan jumlahnya?",
        )
      ) {
    ...
    if (labelParts.length > 1) {
      hargaBeli =
        parseInt(labelParts[1].replace(/,/g, "").replace(".", "")) || 0;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        err.message || "Gagal menyimpan data. Periksa koneksi atau data input.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <form className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="md:col-span-2">
              ...
              <Select
                ...
                className="dark:bg-gray-900 mt-1.5"
              />
              {selectedBarang && selectedStock > 0 && (
                <div className="mt-3 p-3 bg-blue-50 border border-blue-300 rounded-lg dark:bg-blue-500/10 dark:border-blue-500/30">
              ...
              <Table className="w-full">
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {details.map((item, index) => {
                    const hargaJual = hitungHargaJual(
                      item.harga_beli,
                      persenMargin,
                    );
        ...
        {error && (
          <div className="p-4 bg-error-50 border border-error-500 rounded-lg dark:bg-error-500/15">
            ...
            <p className="text-sm text-error-600 dark:text-error-400 ml-7">
              {error}
            </p>
          </div>
        )}
