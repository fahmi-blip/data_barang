...
import { ViewDetailPengadaan } from "../../types/data"; // Perlu ditambahkan di types
import { fetchDetailPengadaanData } from "../../services/DataMasterServices"; // Perlu ditambahkan di services

export default function DetailPengadaanPage() {
  const navigate = useNavigate();
  const [detailList, setDetailList] = useState<ViewDetailPengadaan[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      setError(null);
      try {
        const data = await fetchDetailPengadaanData();
        setDetailList(data);
      } catch (err: any) {
  ...
  return (
    <>
      <PageMeta title={`Detail Pengadaan`} description="Detail item pada transaksi pengadaan." />
      <PageBreadcrumb pageTitle={`Detail Pengadaan`} />
      
      <div className="space-y-6">
        <ComponentCard title="Daftar Barang Pengadaan">
              ...
              <div className="max-w-full overflow-x-auto">
                <Table className="w-full">
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    {detailList.map((item) => (
                      <TableRow key={item.iddetail_pengadaan} className="hover:bg-gray-50 dark:hover:bg-white/5">
                        <TableCell className="px-5 py-4">{item.iddetail_pengadaan}</TableCell>
                        ...
                        <TableCell className="px-5 py-4">{item.sub_total}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
